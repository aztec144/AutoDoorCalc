      ...
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {DOOR_TYPES_CONFIG.map(({ group, types }) => {
            const isGroupActive = types.some(t => t.id === config.doorType);
            
            return (
              <div key={group} className="space-y-3">
                 <div className="relative inline-block mb-1">
                    <h4 className={`text-sm font-bold uppercase tracking-wider pl-1 transition-colors duration-300 ${isGroupActive ? 'text-indigo-600' : 'text-slate-600'}`}>
                      {group}
                    </h4>
                    <div className={`absolute -bottom-1 left-1 h-0.5 bg-indigo-600 rounded-full transition-all duration-500 ease-out ${isGroupActive ? 'w-[calc(100%-4px)]' : 'w-0'}`}></div>
                 </div>
